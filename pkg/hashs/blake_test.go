...
func TestPluginBLAKE2sProcessStreamWithCliFlagsFunc(t *testing.T) {
	p := NewPluginBLAKE2s()
	r := bytes.NewReader(blakeTestData)
	b := flag.NewFlagSet(p.Name, flag.ExitOnError)
	b.String("key", "", "MAC key")
	b.Parse([]string{"--key", blakeTestKey})
	d, e := p.ProcessStreamWithCliFlagsFunc(b, r)
	if e != nil {
...
func TestPluginBLAKE2s128ProcessStreamFunc(t *testing.T) {
	...
	if c := bytes.Compare(d, []byte("64db41748bdb82ee4ae162a8bdc911c7")); c != 0 {
...
func TestPluginBLAKE3512ProcessStreamWithCliFlagsFunc(t *testing.T) {
	...
	r := bytes.NewReader(blakeTestData)
	b := flag.NewFlagSet(p.Name, flag.ExitOnError)
	b.String("derive-key", "", "derive key")
	b.Parse([]string{"--derive-key", blakeTestKey})
	d, e := p.ProcessStreamWithCliFlagsFunc(b, r)
	if e != nil {
		t.Errorf("BLAKE3512ProcessStreamWithCliFlagsFunc failed: %s", e)
	}
	if c := bytes.Compare(d, []byte("96d358355b06b926e7b7f2aa7d654c731c79b1bada39ab0d24fb93ba42a86972e5112ad3ba65187cfd6a15ea62bfd8aff928aa62fe8bd61d1202660ffbb73581")); c != 0 {
