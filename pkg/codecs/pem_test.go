...
func TestPluginPEMProcess(t *testing.T) {
	decodedDER, err := base64.StdEncoding.DecodeString(certDERB64)
	if err != nil {
		t.Fatal(err)
	}
	p := NewPluginPEM()
	assertCodec(t, p, p.Process, decodedDER, []byte(certPEM), "-cert")
}

func TestPluginPEMUnprocess(t *testing.T) {
	decodedDER, err := base64.StdEncoding.DecodeString(certDERB64)
	if err != nil {
		t.Fatal(err)
	}
	p := NewPluginPEM()
	if got := runCodec(t, p.Unprocess, p.RegisterFlags, []byte(certPEM)); !bytes.Equal(got, decodedDER) {
		t.Errorf("PEM decode mismatch")
	}
}
